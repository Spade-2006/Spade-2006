...
🧠 DSA • Systems • Software Engineering  
...
`REST APIs` • `Authentication` • `Authorization` • `API Design` • `Backend Architecture`
...
![Phaser](https://img.shields.io/badge/Phaser-1E1E1E?style=for-the-badge)
![Tiled](https://img.shields.io/badge/Tiled_Map_Editor-2D2D2D?style=for-the-badge)

`Phaser` • `Tiled` • `Tilemaps` • `Game Systems` • `Game Logic`
...
`Arrays` • `Linked Lists` • `Stacks` • `Queues` • `Trees` • `Graphs` • `Hashing` • `Recursion` • `Dynamic Programming` • `Sorting` • `Searching`

### Core CS

`OOP` • `DBMS` • `Operating Systems` • `Computer Networks` • `Computer Architecture`

### Software Engineering

`LLD` • `System Design` • `REST API Design` • `Database Design` • `Object-Oriented Design`
...
Turning Data Structures & Algorithms into quests, puzzles, exploration, and integrated coding trials.
...
Combining geospatial data, maps, and LLM-based reasoning to analyze congestion and generate adaptive traffic recommendations.
...
<img src="https://streak-stats.demolab.com?user=Spade-2006&theme=tokyonight&hide_border=true"/>
